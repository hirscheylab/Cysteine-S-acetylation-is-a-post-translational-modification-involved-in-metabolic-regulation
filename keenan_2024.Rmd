...
Analysis of the cysteine acetylome following subcellular fractionation of mouse liver into cytoplasmic, mitochondrial, and nuclear fractions. 
...
heatmap_function(df = subcell_peptide_norm,
                 ...
                 sample = c(rep("Cyto", 3), rep("Mito", 3), rep("Nuc", 3)))
```
...
rownames(subcell_peptide_pca_norm) <- c(rep("Cyto", 3), rep("Mito", 3), rep("Nuc", 3))
...
subcell_group <- c(rep("Cyto", 3), rep("Mito", 3), rep("Nuc", 3))
subcell_group <- factor(subcell_group,
                                levels = c("Cyto", "Mito", "Nuc"))
subcell_design <- model.matrix(~0 + subcell_group)
colnames(subcell_design) <- c("Cyto_mean", "Mito_mean", "Nuc_mean")
...
subcell.cont <- makeContrasts(
  Mito_v_Cyto = Mito_mean - Cyto_mean,
  Nuc_v_Cyto = Nuc_mean - Cyto_mean,
  Nuc_v_Mito = Nuc_mean - Mito_mean,
...
# bind results
# for Mito vs Cyto
mito_vs_cyto_results <- dplyr::bind_rows(topTable(subcell_var_efit, 
         coef = "Mito_v_Cyto",
...
# for Nuc vs Cyto
nuc_vs_cyto_results <- dplyr::bind_rows(topTable(subcell_var_efit, 
         coef = "Nuc_v_Cyto",
         number = nrow(subcell_peptide_norm_var_mat)),
         topTable(subcell_no_var_efit, 
         coef = "Nuc_v_Cyto",
...
# for Nuc vs Mito
nuc_vs_mito_results <- dplyr::bind_rows(topTable(subcell_var_efit, 
         coef = "Nuc_v_Mito",
         number = nrow(subcell_peptide_norm_var_mat)),
         topTable(subcell_no_var_efit, 
         coef = "Nuc_v_Mito",
...
mito_vs_cyto_results$adj.P.Val <- p.adjust(mito_vs_cyto_results$P.Value,
                                              method = "BH")
nuc_vs_cyto_results$adj.P.Val <- p.adjust(nuc_vs_cyto_results$P.Value,
                                              method = "BH")
nuc_vs_mito_results$adj.P.Val <- p.adjust(nuc_vs_mito_results$P.Value,
                                              method = "BH")
...
# join
# for Mito vs Cyto
mito_vs_cyto_results_join <- mito_vs_cyto_results %>% 
  dplyr::inner_join(protein_site, by = "unique_id") %>% 
  dplyr::distinct()
# for Nuc vs Cyto
nuc_vs_cyto_results_join <- nuc_vs_cyto_results %>% 
  dplyr::inner_join(protein_site, by = "unique_id") %>% 
  dplyr::distinct()
# for Nuc vs Mito
nuc_vs_mito_results_join <- nuc_vs_mito_results %>% 
  dplyr::inner_join(protein_site, by = "unique_id") %>% 
  dplyr::distinct()
# save results
# write_csv(mito_vs_cyto_results_join, "results/mito_vs_cyto_limma.csv")
# write_csv(nuc_vs_cyto_results_join, "results/nuc_vs_cyto_limma.csv")
# write_csv(nuc_vs_mito_results_join, "results/nuc_vs_mito_limma.csv")
# all last saved on 04/30/2024

# plots
mito_vs_cyto_results_join %>% 
  ...
  geom_text_repel(data = . %>% filter(logFC < -2 & adj.P.Val < 0.1), aes(label = protein_mod),
            vjust = -1, hjust = -0.1) +
  labs(title = "Mito vs Cyto") +
  theme_bw() +
  scale_x_continuous(limits = c(-10, 10)) +
  scale_y_continuous(limits = c(0, 4)) 

nuc_vs_cyto_results_join %>% 
  dplyr::mutate(protein_mod = str_remove(protein_mod, "-\\[.+")) %>% 
  dplyr::mutate(protein_mod = str_remove(protein_mod, "-NA")) %>% 
  ggplot(aes(logFC, -log10(adj.P.Val))) +
  geom_point(col = "grey") +
  ...
  geom_text_repel(data = . %>% filter(logFC < -2 & adj.P.Val < 0.1), aes(label = protein_mod),
            vjust = -1, hjust = -0.1) +
  labs(title = "Nuc vs Cyto") +
  theme_bw() +
  scale_x_continuous(limits = c(-10, 10)) +
  scale_y_continuous(limits = c(0, 4)) 

nuc_vs_mito_results_join %>% 
  dplyr::mutate(protein_mod = str_remove(protein_mod, "-\\[.+")) %>% 
  dplyr::mutate(protein_mod = str_remove(protein_mod, "-NA")) %>% 
  ggplot(aes(logFC, -log10(adj.P.Val))) +
  geom_point(col = "grey") +
...
mito_group <- subcel_venn[subcel_venn[[3]] != "Not Found", ]$peptide_id
cyto_group <- subcel_venn[subcel_venn[[2]] != "Not Found", ]$peptide_id
nuc_group <- subcel_venn[subcel_venn[[4]] != "Not Found", ]$peptide_id
  
# Venn diagram
venn.diagram(
  x = list(mito_group, cyto_group, nuc_group),
  category.names = c("Mito" , "Cyto" , "Nuc"),
  filename = 'results/subcell_frac_venn_diagram.png',
  output = TRUE
)
```
...
Cysteine acetylome in BAT from mice acclimated to 4⁰C compared to mice housed at RT. 

### Import data

```{r message = FALSE, warning = FALSE}
# import peptide-level data
rt_vs_fourdeg_peptide <- read_excel("Proteomics Data/All Acetyl-Cys Peptide groups _ all columns _ 20230212EKK20230126_BAT_4degVsRT_PD20230217.xlsx")

# Add a unique identifier to each row
rt_vs_fourdeg_peptide$peptide_id <- paste0("analyte_", 1:nrow(rt_vs_fourdeg_peptide))

# import protein-level data
rt_vs_fourdeg_protein <- read_excel("Protein data/All Proteins _ all columns _ 20230212EKK20230126_BAT_4degVsRT_PD20230217.xlsx")

# import FULL peptide-level data
rt_vs_fourdeg_peptide_all <- read_excel("Proteomics Data/All Peptide groups _ all columns _ 20230212EKK20230126_BAT_4degVsRT_PD20230217.xlsx")

# In the case of peptide-level data, there are some peptides that match to MULTIPLE proteins
# Decision - treat these as DIFFERENT peptides

# select relevant columns and perform mean imputation by group for peptide df
rt_vs_fourdeg_peptide <- rt_vs_fourdeg_peptide %>% 
              dplyr::select(peptide_id, `Master Protein Accessions`, `Modifications in Master Proteins (all Sites)`, starts_with("Abundance:")) %>% 
              dplyr::rowwise() %>% 
              dplyr::mutate(row_means_4deg = mean(c_across(contains("4deg")), na.rm = TRUE),
                            row_means_RT = mean(c_across(contains("RT")), na.rm = TRUE)) %>% 
              dplyr::ungroup() %>% 
              dplyr::mutate(across(contains("4deg"), ~ coalesce(., row_means_4deg)),
...
colsums <- rt_vs_fourdeg_peptide_all %>% 
  dplyr::select(starts_with("Abundance:")) %>% 
  colSums(., na.rm = TRUE)
...
rt_vs_fourdeg_protein <- rt_vs_fourdeg_protein %>% 
                            dplyr::select(Accession, starts_with("Abundance:")) %>% 
                            dplyr::rowwise() %>% 
                            dplyr::mutate(row_means_4deg = mean(c_across(contains("4deg")), na.rm = TRUE),
                                          row_means_RT = mean(c_across(contains("RT")), na.rm = TRUE)) %>% 
...
# reshape peptide data frame
rt_vs_fourdeg_peptide_long <- rt_vs_fourdeg_peptide %>%
  tidyr::separate_longer_delim(`Master Protein Accessions`,
                               delim = ";") %>%
  tidyr::pivot_longer(
    cols = starts_with("Abundance"),
...
# reshape protein data frame
rt_vs_fourdeg_protein_long <- rt_vs_fourdeg_protein %>% 
  tidyr::pivot_longer(cols = starts_with("Abundance"),
                      names_to = "sample_name",
                      values_to = "protein_value")

# join data frames, normalize peptide value, and reshape
rt_vs_fourdeg_peptide_norm <- rt_vs_fourdeg_peptide_long %>% 
  dplyr::inner_join(rt_vs_fourdeg_protein_long,
                    by = c("Accession", "sample_name")) %>% 
  dplyr::mutate(normalized_value = peptide_value/protein_value,
                   .keep = "unused") %>%
  tidyr::pivot_wider(
...
# create a column called `zero_var` that identifies those rows with 0 variance i.e. all 1s
rt_vs_fourdeg_peptide_norm <- rt_vs_fourdeg_peptide_norm %>% 
                                  dplyr::mutate(zero_var = if_all(starts_with("Abundance:"), ~ . == 1))

rt_vs_fourdeg_peptide_norm_var <- rt_vs_fourdeg_peptide_norm %>% 
                                    dplyr::filter(!zero_var) %>% 
                                    dplyr::select(-zero_var) %>% 
                                    dplyr::select(-`Modifications in Master Proteins (all Sites)`)
rt_vs_fourdeg_peptide_norm_no_var <- rt_vs_fourdeg_peptide_norm %>% 
                                        dplyr::filter(zero_var) %>% 
                                        dplyr::select(-zero_var) %>% 
                                        dplyr::select(-`Modifications in Master Proteins (all Sites)`)# empty df

rt_vs_fourdeg_peptide_only <- rt_vs_fourdeg_peptide %>% 
  tidyr::separate_rows(`Master Protein Accessions`) %>% 
  tidyr::unite("unique_id", peptide_id:`Master Protein Accessions`) %>% 
  dplyr::filter(unique_id %in% rt_vs_fourdeg_peptide_norm_no_var$unique_id) %>% 
  dplyr::select(-`Modifications in Master Proteins (all Sites)`)

# combine data frames
# NOTE: treat separately for limma!
rt_vs_fourdeg_peptide_norm <- dplyr::bind_rows(rt_vs_fourdeg_peptide_norm_var,
                                                 rt_vs_fourdeg_peptide_only)
```

### Heatmap

```{r message = FALSE, warning = FALSE}
heatmap_function(df = rt_vs_fourdeg_peptide_norm,
                 cols = c(5,6,7,2,3,4),
                 log_transform = FALSE,
                 cluster_number = 6,
                 distance_metric_row = "correlation",
                 distance_metric_col = "correlation",
                 agglomeration_method = "average",
                 sample = factor(c(rep("RT", 3), rep("4deg", 3)),
                                 levels = c("RT", "4deg"))
                 )
```
...
```{r message = FALSE, warning = FALSE}
rt_vs_fourdeg_peptide_pca_norm <- rt_vs_fourdeg_peptide_norm[2:7] %>% 
                                  tidyr::drop_na() %>% 
                                  as.matrix() 
rt_vs_fourdeg_peptide_pca_norm <- t(rt_vs_fourdeg_peptide_pca_norm)
rownames(rt_vs_fourdeg_peptide_pca_norm) <- c(rep("4deg", 3), rep("RT", 3))
rt_vs_fourdeg_peptide_res.pca_norm <- PCA(rt_vs_fourdeg_peptide_pca_norm,  graph = FALSE)

# Visualize eigenvalues/variances
fviz_screeplot(rt_vs_fourdeg_peptide_res.pca_norm, addlabels = TRUE, ylim = c(0, 75))

fviz_pca_ind(rt_vs_fourdeg_peptide_res.pca_norm, 
             habillage = as.factor(rownames(rt_vs_fourdeg_peptide_pca_norm)),
             repel = TRUE # Avoid text overlapping (slow if many points)
             )
```
...
# first on set normalized to protein levels
# prepare matrix for limma and log2 transform
rt_vs_fourdeg_peptide_norm_var_mat <- as.matrix(rt_vs_fourdeg_peptide_norm_var[ , -1])
rt_vs_fourdeg_peptide_norm_var_mat <- apply(rt_vs_fourdeg_peptide_norm_var_mat, 2, log2)
rownames(rt_vs_fourdeg_peptide_norm_var_mat) <- rt_vs_fourdeg_peptide_norm_var$unique_id

# create design matrix
rt_vs_fourdeg_group <- c(rep("4deg", 3), rep("RT", 3))
rt_vs_fourdeg_group <- factor(rt_vs_fourdeg_group,
                                levels = c("RT", "4deg"))
rt_vs_fourdeg_design <- model.matrix(~rt_vs_fourdeg_group)
colnames(rt_vs_fourdeg_design) <- c("RT_mean", "4deg_vs_RT")

# fit model
rt_vs_fourdeg_peptide_norm_var_mat_fit <- limma::lmFit(rt_vs_fourdeg_peptide_norm_var_mat, rt_vs_fourdeg_design)
rt_vs_fourdeg_peptide_norm_var_mat_efit <- limma::eBayes(rt_vs_fourdeg_peptide_norm_var_mat_fit)

# combine results
# note: here we're not combining anything because there are no "no var" analytes, unlike with the other rt_vs_4deg analysis
# however, the object names are kept the same for simplicity
rt_vs_fourdeg_combo <- topTable(rt_vs_fourdeg_peptide_norm_var_mat_efit, 
         coef = "4deg_vs_RT",
         number = nrow(rt_vs_fourdeg_peptide_norm_var_mat)) %>% 
  data.frame() %>% 
  tibble::rownames_to_column("unique_id") %>% 
  dplyr::mutate(unique_id = str_replace(unique_id, "(?<=\\d)_(?=\\w)", "-")) %>% 
  dplyr::mutate(unique_id = str_replace(unique_id, "(?<=\\d)-(?=\\d)", "_"))
...
# join
rt_vs_fourdeg_combo_join <- rt_vs_fourdeg_combo %>% 
  dplyr::inner_join(protein_site, by = "unique_id") %>% 
  dplyr::distinct()

# save results
# write_csv(rt_vs_fourdeg_combo_join, "results/rt_vs_4deg_mean_imputation_limma.csv")
# last saved on 04/14/2024

rt_vs_fourdeg_combo_join %>% 
  dplyr::mutate(protein_mod = str_remove(protein_mod, "-\\[.+")) %>% 
  ggplot(aes(logFC, -log10(adj.P.Val))) +
  geom_point(col = "grey") +
  geom_vline(xintercept = -2, linetype = 2) +
  ...
  geom_text_repel(data = . %>% filter(logFC < -2 & adj.P.Val < 0.05), aes(label = protein_mod),
            vjust = -1, hjust = -0.1) +
  labs(title = "4deg vs RT") +
  theme_bw()

# visualize abundance of top analytes
rt_vs_fourdeg_peptide_norm %>% 
  dplyr::mutate(unique_id = str_replace(unique_id, "(?<=\\d)_(?=\\w)", "-")) %>% 
  dplyr::mutate(unique_id = str_replace(unique_id, "(?<=\\d)-(?=\\d)", "_")) %>% 
  dplyr::inner_join(protein_site, by = "unique_id") %>% 
...
### Pathway Analysis
```{r message = FALSE, warning = FALSE}
rt_vs_fourdeg_peptide_norm_fgsea <- topTable(rt_vs_fourdeg_peptide_norm_var_mat_efit, 
         coef = "4deg_vs_RT",
         number = nrow(rt_vs_fourdeg_peptide_norm_var_mat)) %>% 
  data.frame() %>% 
  tibble::rownames_to_column("unique_id") %>% 
...
rt_vs_fourdeg_peptide_norm_entrez <- select(org.Mm.eg.db, rt_vs_fourdeg_peptide_norm_fgsea$UNIPROT, "ENTREZID", "UNIPROT")

# join
rt_vs_fourdeg_peptide_norm_entrez_ranks <- rt_vs_fourdeg_peptide_norm_fgsea %>% 
                          dplyr::inner_join(rt_vs_fourdeg_peptide_norm_entrez, by = "UNIPROT") %>% 
                          dplyr::select(ENTREZID, mean_t) %>% 
                          dplyr::arrange(desc(mean_t)) %>% 
                          tidyr::drop_na()

rt_vs_fourdeg_peptide_norm_entrez_ranks <- deframe(rt_vs_fourdeg_peptide_norm_entrez_ranks)
...
set.seed(42)
fgseaRes_rt_vs_fourdeg_peptide_norm <- fgseaMultilevel(pathways = pathways.mh.all.v2023, 
                                            stats = rt_vs_fourdeg_peptide_norm_entrez_ranks)

fgseaResTidy_rt_vs_fourdeg_peptide_norm  <- fgseaRes_rt_vs_fourdeg_peptide_norm %>%
                                    tibble::as_tibble() %>%
                                    dplyr::arrange(padj)
# save results
# write_csv(fgseaResTidy_rt_vs_fourdeg_peptide_norm, "results/rt_vs_4deg_fgsea_results_mean_imputation.csv")
```
...
plotEnrichment(pathways.mh.all.v2023[["HALLMARK_ADIPOGENESIS"]],
               rt_vs_fourdeg_peptide_norm_entrez_ranks) +
...
fgseaResTidy_rt_vs_fourdeg_peptide_norm[fgseaResTidy_rt_vs_fourdeg_peptide_norm$pathway == "HALLMARK_ADIPOGENESIS", ]$leadingEdge
# "11429" "11770"
rt_vs_fourdeg_peptide_norm_entrez[rt_vs_fourdeg_peptide_norm_entrez$ENTREZID %in% 
                                    c("11429", "11770"), ]$UNIPROT
# "P04117" "Q99KI0"

# `HALLMARK_FATTY_ACID_METABOLISM`
fgseaResTidy_rt_vs_fourdeg_peptide_norm[fgseaResTidy_rt_vs_fourdeg_peptide_norm$pathway == "HALLMARK_FATTY_ACID_METABOLISM", ]$leadingEdge
# "11429" "16852" "72479"
rt_vs_fourdeg_peptide_norm_entrez[rt_vs_fourdeg_peptide_norm_entrez$ENTREZID %in% 
...
fgseaResTidy_rt_vs_fourdeg_peptide_norm[fgseaResTidy_rt_vs_fourdeg_peptide_norm$pathway == "HALLMARK_OXIDATIVE_PHOSPHORYLATION", ]$leadingEdge
# "11429" "97212"
rt_vs_fourdeg_peptide_norm_entrez[rt_vs_fourdeg_peptide_norm_entrez$ENTREZID %in% 
